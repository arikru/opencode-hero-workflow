...
describe("templates/.opencode/skills/hero-tdd-loop/SKILL.md", () => {
  test("file exists and frontmatter has name 'hero-tdd-loop' with non-empty description", async () => {
    const text = await Bun.file(TEMPLATE_PATH).text();
    const fm = parseFrontmatter(text);
    expect(fm.name).toBe("hero-tdd-loop");
  ...
  test("body contains the load-bearing intent phrases", async () => {
    ...
    expect(body).toContain("Don't muddle through");
  });
});
