...
describe("templates/.opencode/commands/grill.md", () => {
  test("file exists and frontmatter has non-empty description and argument-hint '<topic>'", async () => {
    const text = await Bun.file(TEMPLATE_PATH).text();
    const fm = parseFrontmatter(text);
    expect(typeof fm.description).toBe("string");
    expect(fm.description!.length).toBeGreaterThan(0);
    expect(fm["argument-hint"]).toBe("<topic>");
  });
});
