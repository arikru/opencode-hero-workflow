...
describe("templates/.opencode/skills/hero-kanban/SKILL.md", () => {
  test("file exists and frontmatter has name 'hero-kanban' with non-empty description", async () => {
    const text = await Bun.file(TEMPLATE_PATH).text();
    const fm = parseFrontmatter(text);
    expect(fm.name).toBe("hero-kanban");
  ...
  test("body contains the load-bearing intent phrases", async () => {
    ...
    expect(body).toContain("vertical slice");
    expect(body).toContain("blocker");
    expect(body).toContain("hero:ready");
    expect(body).toContain("Don't fan out tickets");
  });
});
