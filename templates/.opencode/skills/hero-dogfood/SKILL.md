...
3. If the user asked for a report, proceed to the reporting phase.
...
4. **Render the HTML report.**
   ...
   ```bash
   python .opencode/skills/hero-dogfood/scripts/render_report.py /tmp/dogfood-findings-<ISO-timestamp>.json .opencode/dogfood/report-<ISO-timestamp>.html
   ```
          ...
          → Loop exit: list findings, emit verdict
            → Collect findings → session envelope JSON
              → Write to /tmp/dogfood-findings-<timestamp>.json
                → mkdir -p .opencode/dogfood
                  → python .opencode/skills/hero-dogfood/scripts/render_report.py <findings.json> <out.html>
                    → Print file:// path
                      → Print .gitignore tip
```
